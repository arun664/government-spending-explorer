...
import USReportGenerator from './USReportGenerator.jsx'
 ...
 * 
 * Redesigned to match GDP Analysis structure with:
 * - Global insights panel with spending statistics
 * - US region analysis integration
 * - Interactive world map with search functionality
 * - Consistent layout with GDP dashboard
 */
const SpendingAnalysis = () => {
  ...
  const [activeInsightTab, setActiveInsightTab] = useState('global')
  const [globalStats, setGlobalStats] = useState(null)
  const [yearRange, setYearRange] = useState([2018, 2021])
  const [showSpendingView, setShowSpendingView] = useState(false)
  ...
  const loadData = async () => {
    try {
      ...
      if (loadedData.expenses && loadedData.expenses.length > 0) {
        setData(loadedData)
        
        // Process data for analysis
        const processed = await dataProcessor.processSpendingData(loadedData.expenses)
        setProcessedData(processed)
        
        // Calculate global statistics
        calculateGlobalStats(loadedData.expenses)
      } else {
  ...
  // Calculate global spending statistics
  const calculateGlobalStats = useCallback((expenses) => {
    if (!expenses || expenses.length === 0) return

    const validExpenses = expenses.filter(d => d.value && !isNaN(d.value))
    const totalSpending = validExpenses.reduce((sum, d) => sum + d.value, 0)
    const avgSpending = totalSpending / validExpenses.length
    
    // Group by country for country-level stats
    const countryData = d3.group(validExpenses, d => d.countryCode)
    const countryStats = []
    
    countryData.forEach((countryExpenses, countryCode) => {
      const countryTotal = countryExpenses.reduce((sum, d) => sum + d.value, 0)
      const countryAvg = countryTotal / countryExpenses.length
      const countryName = countryExpenses[0]?.countryName || countryCode
      
      countryStats.push({
        code: countryCode,
        name: countryName,
        totalSpending: countryTotal,
        avgSpending: countryAvg,
        dataPoints: countryExpenses.length
      })
    })
    
    // Sort by total spending
    countryStats.sort((a, b) => b.totalSpending - a.totalSpending)
    
    setGlobalStats({
      totalCountries: countryData.size,
      totalSpending: totalSpending,
      avgSpending: avgSpending,
      totalDataPoints: validExpenses.length,
      top10: countryStats.slice(0, 10),
      bottom10: countryStats.slice(-10).reverse()
    })
  }, [])
  ...
  const getRegion = (countryCode) => {
    ...
    return regionMapping[countryCode] || 'Other'
  }
      setError(null)
